...
const ViewClients = () => {
  ...
  var totalamountraw = JSON.stringify({
    id: Id,
  });
  useEffect(() => {
    const getapi = () => {
  ...
  const getwalletamount = () => {
    ...
    fetch(baseUrl + "/getWalletAmountTotal", requestOptionsAmount)
    .then((response) => response.json())
    .then((result) => {
      if (result.status) {
        setTotalAmount(result.wallet);
      }
    })
    .catch((error) => console.log("error", error));
  };
      ...
      <CCard>
        <CCol xs={12} className="merchant-profile-div">
          <CCol xs={3} className="merchant-profile-image-div">
            <CCardImage orientation="top" className="merchant-profile-image" src={result.profilepic} />
          </CCol>
          <CCol xs={9}>
            <CCardBody className="d-flex">
              <CCol xs={9}>
              <CCardTitle className="profile-name">{result.username}</CCardTitle>
              </CCol>
            </CCardBody>
            <CListGroup flush>
              <CListGroupItem className="group-item-profile-page">
                <strong>Spend Amount:<h4 className="client-count-profile-page">${totalamount ? totalamount:0}</h4></strong>
              </CListGroupItem>
              <CListGroupItem className="group-item-profile-page">
              <strong>Email:<h4 className="client-count-profile-page">{result.email}</h4></strong>
              </CListGroupItem>
              <CListGroupItem className="group-item-profile-page">
              <strong>Phone:<h4 className="client-count-profile-page">{result.phone}</h4></strong>
              </CListGroupItem>
            </CListGroup>
          </CCol>
        </CCol>
